		...
		if(isset($GLOBALS['tf']->variables->request['softsubmit']) && !empty($GLOBALS['tf']->variables->request['softsubmit']) && verify_csrf_referrer(__LINE__, __FILE__)) {
			...
			if(!empty($soft_cat)) {
				foreach($soft_cat as $sof_cat => $softs) {
					...
					add_output('<h2 class="subcategory"><img style="padding-right:5px;" src="http://www.softaculous.net/images/cats/php_'.$sof_cat.'.gif'.'" />'.$sof_cat.'</h2><table cellpadding="26px;" style="width:500px;">');$next =0;

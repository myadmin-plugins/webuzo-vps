...
function webuzo_edit_installation($host, $user, $pass, $installation_id) {
		...
		if(isset($GLOBALS['tf']->variables->request['installation_id'])) $installation_id = $GLOBALS['tf']->variables->request['installation_id'];
		$act = 'editdetail';
		$last_params = "&insid=$installation_id";
		$response = api_call($host, $user, $pass, $act, $last_params);
		...
		if(isset($GLOBALS['tf']->variables->request['editins']) && verify_csrf_referrer(__LINE__, __FILE__)) {
			...
			$response_update = api_call($host, $user, $pass, $act, $last_params ,$post);

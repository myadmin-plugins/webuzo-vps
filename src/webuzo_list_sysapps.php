...
function webuzo_list_sysapps($host=null, $user=null, $pass=null)
{
	...
	if (!empty($final['apps'])) {
		foreach ($final['apps'] as $sof_cat => $softs) {
			add_output('<h2 class="subcategory">'.$sof_cat.'</h2><table cellpadding="26px;" style="width:500px;">');
			$next =0;
			foreach ($softs as $sid => $softw) {
				...
				add_output('<td><a target="SERVICEFrame1" href="iframe.php?choice=none.webuzo_scripts&action=webuzo_view_sysapps&script_id='.$sid.'&vps_id='.$vps_id.'" title="'.htmlentities($softw['desc'], ENT_QUOTES, 'UTF-8').'"><img style="width:100px;height:100px;" src="https://images.softaculous.com/webuzo/softimages/'.$sid.'__'.$softw['logo'].'"/><div style="text-align: left;">'.$softw['name'].'</div></a></td>');
				if ($next == 4) {
					add_output('</tr>');
					$next =0;
				}
